...
- **FEATURES.md** — 実装済み機能の一覧（分野別・「なぜそうしたか」つき）
...
## 運用の分担（Season14 → Season15 の移行期）

**移行は完了済み。** Season14 は13チームの登録・賞金・罰金を入れたうえで
`closeSeason` を実行し、status=終了。予算は10%控除のうえ Season15 へ繰越済み。
以下は当時の分担の記録で、Season15 からは代理入力をしない。

**Season14 はツールを使う前のシーズン。** 試合結果も順位も入っていないので、
必要な数字は主催者が画像やテキストで渡し、**Claude が代わりに入力する**。

| 対象 | 誰がやるか |
|---|---|
| Season14 終了時のスカッド登録（継続参加チーム） | 主催者が画像を送り、**Claude が `importRoster` で入力** |
| Season14 の賞金・罰金 | 主催者が金額を送り、**Claude が入力** |
| Season14 の終了処理 | **Claude** が数字を入れたうえで `closeSeason` を実行 |
| **Season15 以降の終了処理** | **ツール内で完結**。試合結果から自動計算する |

Season15 からは順位表も賞金も自動で出るので、代理入力はしない。
`closeSeason` を押すだけで、賞金 → スポンサー判定 → 手数料 → 繰越まで通る。

エントリーリスト画像の読み取り手順は **OPERATION.md §1-4.6** に定型化してある。
記号（△外国籍 / 下線U-21 / [ ]プロテクト / 太字=移籍加入）の件数を
ヘッダーや各一覧と照合し、**4つとも一致すること**を確認してから登録する。

## データの正（どちらを採るか）

**主催者が作る eFootball の名簿が一次資料。**
年齢・国籍・現実クラブ・ポジションは `syncPlayerProfiles` で名簿の値に上書きする。
エントリーリストの表記と食い違ったら名簿を採る。参加者が毎シーズン書くものなので
表記が揺れるため。`importRoster`（エントリーリストの取り込み）は逆に、
既にいる選手の項目が**空欄のときだけ**埋める。

**現実クラブ名はチーム名と完全一致させる。** 補填の入れ替え候補は
`real_club === チーム名` で絞るので、「町田ゼルビア」と「FC町田ゼルビア」のように
一字違うと候補が1件も出なくなる。名簿を受け取ったら、まず
参加13チームの名前と突き合わせて確認する。

**人名の異体字は名寄せで吸収する。** 髙/高、﨑/崎、濱/浜 など。
`_normalizeName` の `NAME_VARIANTS` に対応表がある。
畳み込まないと同じ選手が二重に登録される（実際に6組出た）。

## スカッド人数

**22〜35名はリーグのルールとして維持する。** その範囲内で何名にするかは
参加者の自由で、主催者は関与しない。人数が多い・少ないことを問題として
報告する必要はない。上限・下限は Config の `squad_min` / `squad_max`。

## 表記のルール

- **例示に主催者の本名（ひなた）を使わない。** プレースホルダやサンプルは
  「GM太郎」などの架空名を使う。GitHub の URL に含まれるアカウント名は
  実在のインフラなので変更しない。
- **選手名・クラブ名は実在のものを使う。** ここは架空名に置き換えない。
...
│   ├─ setupSheets.gs  # 全25シート作成・Config / Clubs 初期値投入（冪等）
