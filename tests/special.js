...
t('在籍の獲得形態だけでも弾く（期限付き・オークション）', () => {
  // 主催者が取り込みで直接入れた在籍には Transfers の履歴が無い。
  // この3形態はシーズン末に必ず離脱するので、在籍にあれば今シーズンの行と確定できる
  const e = heldByA(market(), '全期期限付き');
  eq(grab(e, '特別').ok, false);
});

t('前シーズンに特別で獲った選手は在籍の獲得形態だけでは弾かない', () => {
  // 引継ぎは acquisition_type を次シーズンへ保持する（補填金の母数になるため）。
  // これを「今シーズン動いた」と読むと、一度強奪された選手が永久に守られてしまう
  const e = heldByA(market(), '特別');
  eq(grab(e, '特別').ok, true, '前季の獲得形態で今季の可否を決めてはいけない');
});
