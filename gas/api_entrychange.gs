 ...
 *
 * ▶ 外せる選手
 *   - 自チームの在籍（または申請中）で、現実クラブが自クラブ
 *   - 大会対象（eligible）で、補填の請求が立っていない
 *   - 期限付き・オークションで預かっている選手ではない
 *   - プロテクトしていない
 *
 *   他クラブの選手を外せないのは、無償で手放すと移籍市場を通さずに
 *   その選手のクラブのGMへ渡る抜け道になるため。放出は移籍市場で行う。
 *
 * ▶ 入れられる選手
...
function _entryChangeOutBlock(ctx, roster, teamId, myClub) {
  ...
  if (_str(p.real_club) !== myClub) {
    return "他クラブの選手は外せません。放出は移籍市場で行ってください。";
  }
