...
function _getSheetDefinitions() {
  return [
    ...
    {
      // §4.24 Sponsors ─ スポンサーのマスタ（シーズンごとに設定）
      name: "Sponsors",
      spec: "SPEC.md §4.24",
      headers: [
        "sponsor_id",   // string  主キー
        "season_id",    // string  このシーズン用の設定
        "name",         // string  スポンサー名
    ...
    {
      // §4.25 TeamSponsors ─ チームが契約したスポンサー（1チーム1社）
      name: "TeamSponsors",
      spec: "SPEC.md §4.25",
      headers: [
        "contract_id",  // string    主キー
        "season_id",    // string
        "team_id",      // string
        "sponsor_id",   // string
        "contract_fee", // number    契約時点の契約金（後から設定を変えても記録は動かさない）
        "chosen_at",    // datetime
        "result",       // enum      未判定 / 達成 / 未達
        "penalty_paid", // number    実際に引かれた罰金
        "settled_at",   // datetime  判定した日時
      ],
    },
    {
 ...
 *
 * ⚠️ 前シーズン（6/1開幕）の日付から逆算した実績値。
 *    プロテクトの掲示は画面でリアルタイムに見られるので、予定には入れていない。
 *
 * @returns {{ added: number, skipped: number }}
 */
function _setupScheduleTemplate() {
  ...
  // 開幕 = 0。逆算して並べている
  var defs = [
    [-23, "継続参加者の募集期限", "継続の意思確認。返答が無ければ辞退として扱う"],
    [-18, "新規募集終了", "X での募集を締め切る。登録リンクは応募者へ個別に送る"],
    [-17, "エントリー変更開始", ""],
    [-16, "使用監督申告開始", ""],
    [-16, "無料プロテクト開始", "掲示は「プロテクト」タブでリアルタイムに見られる"],
