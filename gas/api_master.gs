 ...
 * 大会で扱う金額の最小単位（円）。
 *
 * 移籍金も賞金も 100万円単位でやり取りするリーグなので、
 * 率をかけて出た端数もここに揃える。
 */
var MONEY_UNIT = 1000000;

/**
 * 計算で出た金額を 10万の位で四捨五入して 100万円単位に揃える。
 *
 * ▶ なぜ必要か
 *   補填金（獲得額×80%）やシーズン終了手数料（残予算×10%）のように
 *   率をかける計算は、そのままだと 2,640万 や 80万 のような端数が出る。
 *   大会の最小単位は 100万円なので、参加者が扱えない額になってしまう。
 *
 * ⚠️ 率をかけて**計算した結果**にだけ使う。
 *   交渉額や獲得額のように主催者・参加者が直接入れた金額は丸めない。
 *   入れた数字が黙って変わると、何を合意したのか追えなくなる。
 *
 * 例: 2,640万 → 2,600万 / 80万 → 100万 / 6,400万 → 6,400万
 *
 * @param {*} v
 * @returns {number}
 */
function _roundMoney(v) {
  return Math.round(_num(v) / MONEY_UNIT) * MONEY_UNIT;
}
