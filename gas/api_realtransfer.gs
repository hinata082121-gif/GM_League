 ...
 *   releaseToLeagueClub    — 新規参加クラブへ移った選手。手放させるが大会には残る
 ...
 *
 * ▶ 現実の移籍とリーグ内の保有は別の話
 *   移籍先が参加クラブなら、その選手はこれまでどおり保有GMの在籍のまま。
 *   町田の選手が神戸へ移っても、FC東京が持っているならFC東京の在籍で変わらない。
 *   リーグ内の保有は移籍市場で決まるものだからである。
 *
 *   保有を動かすのは次の2つだけ。
 *     参加クラブ外へ出た   → applyRealTransfers
 *     新規参加クラブへ移った → releaseToLeagueClub
 *
 * ▶ 確定仕様
 ...
 * 人数が減るため、金額と人数を先に見せる。
 *
 * ⚠️ これは「保有チーム≠現実クラブ」を全件返す。移籍市場で獲得した選手も
 *   混ざるので、返ってきたものを一括で流してはいけない。
 *   現実で移籍した選手だけを選ぶこと。
 *
...
/**
 * 新規参加クラブへ移った選手を、今の保有チームから手放させる。主催者専用。
 *
