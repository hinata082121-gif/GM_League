...
### 6.5 現実移籍の扱い
- 現実の移籍はスカッドに反映しない。
- ただし大会参加外チームへ移籍した選手は Players.eligible=false とし、翌シーズンから使用不可。

**確定仕様（反映の手順）**

主催者が「運営・進行 → 現実移籍の反映」で対象選手を選び、まとめて反映する。
`applyRealTransfers` が以下を1トランザクションで行う。

1. `Players.eligible` を false にする
2. **その場で補填金を計上する**（`acquired_cost × compensation_rate_transfer`、既定80%）
3. 今シーズンの Rosters と試合結果には**一切触れない**

`eligible=false` の選手は次の3か所で締め出される。

| 場所 | 挙動 |
|---|---|
| エントリー提出 | 候補に出さない・提出時も拒否（従来どおり） |
...
>
> 補填金を反映と同時に計上するのは、`eligible` を外す操作と補填を分けると
> 計上漏れが起きるため。1つの操作にまとめて漏れをなくす。
>
> オークション獲得の選手は補填の対象外（シーズン終了で自動離脱するため）。
> 獲得額0（エントリーで取った選手）も補填は発生しない。
>
> 誤って外した場合は `restorePlayerEligible` で戻せるが、
> **補填金は自動では取り消さない。** 予算の増減は履歴として残す方針のため、
> 打ち消すなら罰金で相殺する（何が起きたか BudgetTx に残る）。
