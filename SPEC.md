...
6. 残予算 × `season_end_fee_rate` を「シーズン終了手数料」として控除（**1〜5の計上後の残高が母数**）
7. `expires_season` = 当該シーズンの在籍行を `離脱` にする
8. `next_season_id` が指定されていれば、アクティブなチームの在籍選手を次シーズンへコピー
9. シーズンの status を `終了` に
