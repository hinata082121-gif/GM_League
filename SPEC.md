...
### closeSeason の処理内容（Phase 7 で確定）
1. **順位賞金**を BudgetTx に計上。`rank_prize_1` 〜 `rank_prize_3`（Config にある分だけ）。
2. **得点王賞金**を計上。得点1位の選手の**シーズン終了時点の所属チーム**に支給。
3. 残予算 × `season_end_fee_rate` を「シーズン終了手数料」として控除（1・2の計上後の残高が母数）。
4. `expires_season` = 当該シーズンの在籍行を `離脱` にする（オークション・全期期限付きなど）。
5. `next_season_id` が指定されていれば、**アクティブなチームの在籍選手を次シーズンへコピー**する。
6. シーズンの status を `終了` にする。

**確定した挙動**

- **同順位・同点は全チームに全額支給。** 1位が2チーム並べば両方に `rank_prize_1` を満額。
  得点王が複数いれば該当チーム全てに `top_scorer_prize` を満額。按分はしない。
- **次シーズンは主催者が先に作成しておく。** closeSeason は `next_season_id` を引数で受け取るだけで、
  シーズン行の自動生成はしない（移籍市場の日程を先に決められるようにするため）。
- **引継ぎ時は `acquisition_type` と `acquired_cost` を保持する。**
  補填金（獲得額 × 80% / 90%）の母数として後から必要になるため。`expires_season` は空にする。
- **二重実行を拒否する。** 既に `シーズン終了手数料` が計上されているシーズンは再実行できない。
  賞金の二重支給を防ぐため。
- スポンサー額は次シーズンの第1次市場前に `applySponsorIncome` で入力する。
